/**
 * Commit Window Candidate Strategy
 *
 * Finds delta base candidates from recent commits (sliding window).
 ...
 * Strategy options
 */
export interface CommitWindowCandidateOptions {
...
export class CommitWindowCandidateStrategy implements DeltaCandidateStrategy {
  ...
  constructor(options: CommitWindowCandidateOptions = {}) {
  ...
  async *findCandidates(targetId: ObjectId, _storage: ObjectStorage): AsyncIterable<ObjectId> {
    // Return recent objects (excluding target), most recent first
    let yielded = 0;
    for (let i = this.recentObjects.length - 1; i >= 0 && yielded < this.windowSize; i--) {
