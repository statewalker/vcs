/**
 * Tests for async stream utilities
 */
...
import {
  ...
  splitStream,
  toLines,
} from "../../src/format/async-utils.js";
...
// Helper to encode string to Uint8Array
function _encodeAll(...str: string[]): Uint8Array[] {
  const encoder = new TextEncoder();
  return str.map((s) => encoder.encode(s));
}
...
describe("async-utils", () => {
  ...
  describe("newHeaderDetector", () => {
    it("finds single-byte delimiter in single block", () => {
      const detector = newHeaderDetector(new Uint8Array([0]));
    ...
    it("finds multi-byte delimiter in single block", () => {
      const detector = newHeaderDetector(new Uint8Array([13, 10, 13, 10])); // \r\n\r\n
    ...
    it("finds delimiter spanning two blocks", () => {
      const detector = newHeaderDetector(new Uint8Array([1, 2, 3, 4]));
    ...
    it("handles false start then real match", () => {
      ...
      const detector = newHeaderDetector(new Uint8Array([1, 1, 2]));
    ...
    it("handles empty blocks", () => {
      const detector = newHeaderDetector(new Uint8Array([1, 2]));
    ...
    it("handles delimiter longer than block", () => {
      const detector = newHeaderDetector(new Uint8Array([1, 2, 3, 4, 5]));
    ...
    it("handles single-byte blocks with long delimiter", () => {
      const detector = newHeaderDetector(new Uint8Array([1, 2, 3, 4, 5, 6, 7, 8]));
    ...
    it("handles single-byte blocks with data before delimiter", () => {
      const detector = newHeaderDetector(new Uint8Array([1, 2, 3]));
    ...
    it("handles blocks smaller than delimiter with match at various positions", () => {
      const detector = newHeaderDetector(new Uint8Array([0xaa, 0xbb, 0xcc]));
    ...
    it("handles very long delimiter with tiny blocks", () => {
      ...
      const detector = newHeaderDetector(delimiter);
    ...
    it("works with binary delimiter", () => {
      const detector = newHeaderDetector(new Uint8Array([0xff, 0xfe]));
