import { createFossilLikeRanges, type DeltaRange } from "../../src/index.js";
...
function applyRanges(source: Uint8Array, target: Uint8Array, ranges: DeltaRange[]): Uint8Array {
  ...
  for (const range of ranges) {
    ...
    if (range.from === "source") {
      chunk = source.subarray(range.start, range.start + range.len);
    } else {
      // from === "target"
      chunk = target.subarray(range.start, range.start + range.len);
    }
...
/**
 * Test utility for createFossilLikeRanges
 *
 ...
 */
export function testFossilLikeRanges(options: MutationOptions): PerformanceStats {
  ...
  const ranges = Array.from(createFossilLikeRanges(source, target, blockSize));
  ...
  // const compressionRatio = targetSize > 0 ? rangesSize / targetSize : 1;
