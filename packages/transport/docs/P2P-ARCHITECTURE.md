...
```typescript
async function createGitStreamFromPort(
  port: MessagePortLike,
  options?: PortGitStreamOptions
): Promise<GitBidirectionalStream>;

async function createGitStreamPair(
  options?: PortGitStreamOptions
): Promise<[GitBidirectionalStream, GitBidirectionalStream]>;
```

**Key challenges solved:**

1. **Push-to-pull conversion**: Uses async generators to buffer incoming messages for pull-based iteration
...
4. **Resource cleanup**: Proper cleanup of event listeners and port closure via `stream.close()`
...
const [streamA, streamB] = await createGitStreamPair();
