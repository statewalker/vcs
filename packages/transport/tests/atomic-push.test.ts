...
import { getState, setRefStore } from "../src/context/context-adapters.js";
...
describe("APPLY_UPDATES FSM Handler", () => {
  it("should apply all updates successfully", async () => {
    ...
    expect((ctx as any).pushCommands?.[0]?.result).toBe("OK");
  ...
  it("should rollback on failure with atomic capability", async () => {
    ...
    getState(ctx).capabilities.add("atomic");
    ...
    expect((ctx as any).pushCommands?.[0]?.result).toBe("ATOMIC_REJECTED");
    // Second command should have LOCK_FAILURE
    expect((ctx as any).pushCommands?.[1]?.result).toBe("LOCK_FAILURE");
  });
  ...
  it("should skip already-rejected commands", async () => {
    ...
    expect((ctx as any).pushCommands?.[1]?.result).toBe("REJECTED_NONFASTFORWARD");
