...
describe("fetchFromPeer", () => {
  it("should fetch refs from peer", async () => {
    ...
    const [fetchResult] = await Promise.all([
      ...
      mockUploadPackServer(serverStream.stream, [{ name: "refs/heads/main", id: COMMIT_ID }]),
    ]);
  ...
  it("should handle empty repository", async () => {
    ...
    const [fetchResult] = await Promise.all([
      ...
      mockUploadPackServer(serverStream.stream, []), // Empty repo
    ]);
  ...
  it("should call progress callbacks", async () => {
    ...
    const [fetchResult] = await Promise.all([
      ...
      mockUploadPackServerWithProgress(serverStream.stream, [
...
describe("pushToPeer", () => {
  it("should push refs to peer", async () => {
    ...
    const [pushResult] = await Promise.all([
      ...
      mockReceivePackServer(serverStream.stream),
    ]);
  ...
  it("should handle push rejection", async () => {
    ...
    const [pushResult] = await Promise.all([
      ...
      mockReceivePackServerWithRejection(serverStream.stream),
    ]);
