...
describe("VALIDATE_COMMANDS FSM Handler", () => {
  it("should validate fast-forward updates", async () => {
    ...
    // Set current ref
    ctx.state.refs.set("refs/heads/main", "a".repeat(40));
    ...
    expect(state.pushCommands?.[0]?.result).toBe("NOT_ATTEMPTED"); // Not modified yet
  });
  ...
  it("should reject when old OID doesn't match current ref", async () => {
    ...
    ctx.state.refs.set("refs/heads/main", "c".repeat(40)); // Changed since fetch!
    ...
    expect(state.pushCommands?.[0]?.message).toContain("changed");
  });
  ...
  it("should allow delete when ref exists", async () => {
    ...
    ctx.state.refs.set("refs/heads/to-delete", "d".repeat(40));
  ...
  it("should return SOME_INVALID for mixed results", async () => {
    ...
    ctx.state.refs.set("refs/heads/main", "a".repeat(40));
    ctx.state.refs.set("refs/heads/feature", "x".repeat(40)); // Different from expected
...
describe("CHECK_CONNECTIVITY FSM Handler", () => {
  ...
  it("should fail when new object is missing", async () => {
    ...
    expect(state.pushCommands?.[1]?.result).toBe("REJECTED_MISSING_OBJECT");
  });
...
describe("SEND_STATUS FSM Handler", () => {
  it("should send status for successful push", async () => {
    ...
    ctx.state.capabilities.add("report-status");
  ...
  it("should skip status when report-status not negotiated", async () => {
    ...
    ctx.state.capabilities.clear(); // No report-status
