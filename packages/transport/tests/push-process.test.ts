...
import { getState, setRepository } from "../src/context/context-adapters.js";
...
import { createMockRepository, createTestContext } from "./helpers/test-protocol.js";
...
describe("Ref Deletion", () => {
  ...
  it("should return NON_EXISTING for missing ref", () => {
    ...
    const result = validatePushUpdate(update, remoteRefs, new Set(), new Map());
...
describe("Mixed Updates", () => {
  it("should handle independent updates independently", () => {
    ...
    const results = updates.map((u) => validatePushUpdate(u, remoteRefs, localObjects, ancestors));
...
// These tests verify the real FSM handler behavior for push process
// ─────────────────────────────────────────────────────────────────────────────

describe("VALIDATE_COMMANDS FSM Handler", () => {
  it("should validate fast-forward updates", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    setRepository(ctx, repo);

    // Old OID exists in repo (for fast-forward check)
    repo._addObject("a".repeat(40));
    repo._addObject("b".repeat(40));

    // Set current ref
    getState(ctx).refs.set("refs/heads/main", "a".repeat(40));

    (ctx as any).pushCommands = [
      {
        oldOid: "a".repeat(40),
        newOid: "b".repeat(40),
        refName: "refs/heads/main",
    ...
    const handler = serverPushHandlers.get("VALIDATE_COMMANDS");
    expect(handler).toBeDefined();
    const result = await handler?.(ctx);

    expect(result).toBe("ALL_VALID");
    expect((ctx as any).pushCommands?.[0]?.result).toBe("NOT_ATTEMPTED"); // Not modified yet
  });

  it("should reject when old OID doesn't match current ref", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    setRepository(ctx, repo);

    // Current ref is different from what client thinks
    getState(ctx).refs.set("refs/heads/main", "c".repeat(40)); // Changed since fetch!

    (ctx as any).pushCommands = [
      {
        oldOid: "a".repeat(40), // Client's old OID
        newOid: "b".repeat(40),
        refName: "refs/heads/main",
        type: "UPDATE",
        result: "NOT_ATTEMPTED",
    ...
    expect(result).toBe("ALL_INVALID");
    expect((ctx as any).pushCommands?.[0]?.result).toBe("REJECTED_NONFASTFORWARD");
    expect((ctx as any).pushCommands?.[0]?.message).toContain("changed");
  });

  it("should allow create when ref doesn't exist", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    setRepository(ctx, repo);
  ...
  it("should allow delete when ref exists", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    setRepository(ctx, repo);

    getState(ctx).refs.set("refs/heads/to-delete", "d".repeat(40));
  ...
  it("should return SOME_INVALID for mixed results", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    setRepository(ctx, repo);
    ...
    getState(ctx).refs.set("refs/heads/main", "a".repeat(40));
    getState(ctx).refs.set("refs/heads/feature", "x".repeat(40)); // Different from expected
    ...
    (ctx as any).pushCommands = [
      ...
      {
        oldOid: "c".repeat(40), // Doesn't match current
        newOid: "d".repeat(40),
        refName: "refs/heads/feature",
        type: "UPDATE",
        result: "NOT_ATTEMPTED",
    ...
    expect(result).toBe("SOME_INVALID");
  });
});

describe("CHECK_CONNECTIVITY FSM Handler", () => {
  it("should pass when all new objects exist", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    setRepository(ctx, repo);

    repo._addObject("b".repeat(40));
    repo._addObject("d".repeat(40));
    ...
    const handler = serverPushHandlers.get("CHECK_CONNECTIVITY");
    expect(handler).toBeDefined();
    const result = await handler?.(ctx);

    expect(result).toBe("CONNECTIVITY_OK");
  });

  it("should fail when new object is missing", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    setRepository(ctx, repo);

    // Only first object exists
    repo._addObject("b".repeat(40));
    // "d".repeat(40) is missing!
    ...
    expect(result).toBe("CONNECTIVITY_FAILED");
    expect((ctx as any).pushCommands?.[1]?.result).toBe("REJECTED_MISSING_OBJECT");
  });

  it("should skip connectivity check for deletes", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    setRepository(ctx, repo);

    // No objects needed for delete
    (ctx as any).pushCommands = [
      {
        oldOid: "d".repeat(40),
        newOid: ZERO_OID,
...
describe("SEND_STATUS FSM Handler", () => {
  it("should send status for successful push", async () => {
    const ctx = createTestContext();
    ...
    const handler = serverPushHandlers.get("SEND_STATUS");
    expect(handler).toBeDefined();
    const result = await handler?.(ctx);

    expect(result).toBe("STATUS_SENT");
  });

  it("should skip status when report-status not negotiated", async () => {
    const ctx = createTestContext();

    getState(ctx).capabilities.clear(); // No report-status
