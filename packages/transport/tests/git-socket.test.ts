...
describe("GitSocket Integration", () => {
  ...
  it("should transfer pack data when client has no local commits", async () => {
    ...
    const packDataChunks: Uint8Array[] = [];
    ...
    for await (const packet of packets) {
      if (packet.type === "flush") {
        ...
        const preview = new TextDecoder().decode(
          packet.data.slice(0, Math.min(30, packet.data.length)),
        );
        const firstByte = packet.data[0];
        console.log(
          `Received data packet: len=${packet.data.length}, firstByte=${firstByte}, preview="${preview.replace(/\n/g, "\\n")}"`,
        );
...
describe("GitSocket Multiple Operations", () => {
  it("should support multiple sequential fetch operations on same port", async () => {
    ...
    const repository = createMockRepository({
      refs: [{ name: "refs/heads/main", objectId: COMMIT_A }],
    });
