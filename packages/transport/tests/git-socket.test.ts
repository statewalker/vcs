  ...
  describe("discoverRefs", () => {
    it("should send correct initial request format", async () => {
      ...
      const serverRead = (async () => {
        const packets = pktLineReader(serverSocket.read());
        ...
        await serverSocket.write(encodePacket(`${COMMIT_A} refs/heads/main\0\n`));
        await serverSocket.write(encodeFlush());
...
describe("GitSocketServer", () => {
  describe("handleGitSocketConnection", () => {
    it("should handle valid upload-pack request", async () => {
      ...
      const serverPromise = handleGitSocketConnection(serverSocket, options);

      // Client: send request with full protocol flow
      const clientHandle = (async () => {
        // Send initial request
        await clientSocket.write(encodePacket("git-upload-pack /repo.git\0host=localhost\0"));

        // Read ref advertisement (until flush)
        const packets = pktLineReader(clientSocket.read());
        const received: string[] = [];
        ...
        await clientSocket.write(encodeFlush());
    ...
    it("should handle valid receive-pack request", async () => {
      ...
      const clientHandle = (async () => {
        await clientSocket.write(encodePacket("git-receive-pack /repo.git\0host=localhost\0"));
    ...
    it("should reject invalid service", async () => {
      ...
      const serverPromise = handleGitSocketConnection(serverSocket, options).catch(() => {
        // Expected to throw
      });

      const clientHandle = (async () => {
        await clientSocket.write(encodePacket("git-invalid-service /repo.git\0host=localhost\0"));

        const chunks: Uint8Array[] = [];
        for await (const chunk of clientSocket.read()) {
          chunks.push(chunk);
        }
    ...
    it("should return error for non-existent repository", async () => {
      ...
      const clientHandle = (async () => {
        await clientSocket.write(encodePacket("git-upload-pack /unknown.git\0host=localhost\0"));
    ...
    it("should call authenticate callback", async () => {
      ...
      const clientHandle = (async () => {
        await clientSocket.write(encodePacket("git-upload-pack /repo.git\0host=myhost\0"));
...
describe("GitSocket Integration", () => {
  it("should complete client-server ref discovery", async () => {
    ...
    // Start server
    const serverPromise = handleGitSocketConnection(serverSocket, serverOptions);
