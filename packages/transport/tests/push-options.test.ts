...
describe("READ_PUSH_OPTIONS FSM Handler", () => {
  it("should read push options when capability is negotiated", async () => {
    ...
    ctx.state.capabilities.add("push-options");
    ...
    const state = ctx.state as { pushOptions?: string[] };
    expect(state.pushOptions).toHaveLength(2);
    expect(state.pushOptions).toContain("ci.skip");
    expect(state.pushOptions).toContain("merge.targetBranch=main");
  });
  ...
  it("should handle empty options list", async () => {
    ...
    const state = ctx.state as { pushOptions?: string[] };
    expect(state.pushOptions).toHaveLength(0);
  });
  ...
  it("should preserve option order", async () => {
    ...
    const state = ctx.state as { pushOptions?: string[] };
    expect(state.pushOptions?.[0]).toBe("first");
    expect(state.pushOptions?.[1]).toBe("second");
    expect(state.pushOptions?.[2]).toBe("third");
  });
  ...
  it("should handle options with equals sign in value", async () => {
    ...
    const state = ctx.state as { pushOptions?: string[] };
    expect(state.pushOptions?.[0]).toBe("custom.key=value=with=equals");
  });
});
