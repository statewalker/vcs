...
describe("Smart HTTP Client-Server", () => {
  ...
  describe("redirects", () => {
    ...
    it("should handle 302 redirect", async () => {
      ...
      const response = await server.fetch(
        new Request("http://localhost:3000/temp.git/info/refs"),
      );
    ...
    it("should handle 307 redirect", async () => {
      ...
      const response = await server.fetch(
        new Request("http://localhost:3000/moved.git/info/refs"),
      );
    ...
    it("should handle multiple redirects", async () => {
      const server = TestHttpServer.create({
        redirects: new Map([
          ["/first.git", { status: 301, location: "http://localhost:3000/second.git/info/refs", count: 2 }],
        ]),
      });
...
describe("HTTP Error Handling", () => {
  describe("ProtocolError", () => {
    it("should return proper HTTP error codes", async () => {
      const server = TestHttpServer.create({
        errors: new Map([
          ["/broken.git", { status: 500, message: "Internal error" }],
        ]),
      });
    ...
    it("should include error details in response", async () => {
      const server = TestHttpServer.create({
        errors: new Map([
          ["/error.git", { status: 503, message: "Service Unavailable" }],
        ]),
      });

      const response = await server.fetch(
        new Request("http://localhost:3000/error.git/info/refs"),
      );
  ...
  describe("AdvertiseError", () => {
    it("should handle errors during ref advertisement", async () => {
      const server = TestHttpServer.create({
        errors: new Map([
          ["/fail-advertise.git", { status: 403, message: "Access denied" }],
        ]),
      });
    ...
    it("should report error message to client", async () => {
      const server = TestHttpServer.create({
        errors: new Map([
          ["/denied.git", { status: 403, message: "Repository access denied" }],
        ]),
      });
