 ...
 * 1. Unit tests for individual components (test helpers, TestRepository)
 * 2. Integration tests using real FSM handlers from src/fsm/fetch/
 *
...
// These tests verify the real FSM handler behavior for upload-pack
// ─────────────────────────────────────────────────────────────────────────────

describe("SEND_ADVERTISEMENT FSM Handler", () => {
  it("should send refs advertisement with capabilities", async () => {
    ...
    expect(result).toBe("REFS_SENT");

    const lines = transport._getWrittenLines();
    expect(lines.length).toBeGreaterThan(0);
    // First line should have capabilities (null byte separator)
...
describe("VALIDATE_WANTS FSM Handler", () => {
  it("should accept advertised wants", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    ctx.repository = repo;

    // Populate refs (advertised)
    ctx.state.refs.set("refs/heads/main", "a".repeat(40));

    // Add want for advertised ref
    ctx.state.wants.add("a".repeat(40));
  ...
  it("should reject unadvertised wants by default", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    ctx.repository = repo;

    // No refs - nothing advertised
    ctx.state.refs.clear();

    // Want for non-advertised object
    ctx.state.wants.add("x".repeat(40));

    const handler = serverFetchHandlers.get("VALIDATE_WANTS");
    expect(handler).toBeDefined();
    const result = await handler?.(ctx);

    expect(result).toBe("INVALID_WANT");
  });

  it("should allow any object when policy is ANY", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    ctx.repository = repo;

    // Object exists in repo
    repo._addObject("x".repeat(40));

    // Not advertised but exists
    ctx.state.wants.add("x".repeat(40));
...
describe("SEND_FINAL_ACK FSM Handler", () => {
  it("should send NAK when no common base", async () => {
    const ctx = createTestContext();
    const transport = ctx.transport as MockTransport;

    ctx.state.commonBase.clear(); // No common base

    const handler = serverFetchHandlers.get("SEND_FINAL_ACK");
    expect(handler).toBeDefined();
    const result = await handler?.(ctx);

    expect(result).toBe("NAK_SENT");
  ...
  it("should send ACK with common base OID", async () => {
    ...
    ctx.state.commonBase.add("a".repeat(40));

    const handler = serverFetchHandlers.get("SEND_FINAL_ACK");
    expect(handler).toBeDefined();
    const result = await handler?.(ctx);

    expect(result).toBe("ACK_SENT");

    const lines = transport._getWrittenLines();
    expect(lines.some((l) => l.includes("ACK") && l.includes("a".repeat(40)))).toBe(true);
  });
});

describe("SEND_PACK FSM Handler", () => {
  it("should send pack data", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    ctx.repository = repo;
    ...
    expect(result).toBe("PACK_SENT");
  });

  it("should exclude common base objects from pack", async () => {
    const ctx = createTestContext();
    const repo = createMockRepository();
    ctx.repository = repo;
    ...
    ctx.state.wants.add("a".repeat(40));
    ctx.state.commonBase.add("b".repeat(40)); // Should be excluded
    ...
    expect(result).toBe("PACK_SENT");
    // Pack should be generated (we can't easily verify exclusion without inspecting the pack)
  });
});
