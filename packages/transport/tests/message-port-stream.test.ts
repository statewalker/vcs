...
function createMockPort(): MessagePortLikeExtended & {
  ...
  return {
    ...
    set _isOpen(value: boolean) { isOpen = value; },
    get _bufferedAmount() { return bufferedAmount; },
    set _bufferedAmount(value: number) { bufferedAmount = value; },
    get bufferedAmount() { return bufferedAmount; },
    get isOpen() { return isOpen; },
    postMessage(data: ArrayBuffer | Uint8Array) { sentMessages.push(data); },
    close() { isOpen = false; this.onclose?.(); },
    start() {},
    simulateMessage(data: ArrayBuffer) { this.onmessage?.({ data } as MessageEvent<ArrayBuffer>); },
    simulateClose() { isOpen = false; this.onclose?.(); },
    simulateError(err: Error) { this.onerror?.(err); },
  };
}

function createStream(port: MessagePortLikeExtended, opts?: { highWaterMark?: number; drainInterval?: number }): MessagePortStream {
