...
describe("READ_WANTS FSM Handler", () => {
  it("should parse want with capabilities", async () => {
    ...
    expect(ctx.state.capabilities.has("multi_ack")).toBe(true);
    expect(ctx.state.capabilities.has("thin-pack")).toBe(true);
  });
  ...
  it("should parse multiple wants", async () => {
    ...
    expect(ctx.state.wants.size).toBe(2);
    expect(ctx.state.wants.has(oid1)).toBe(true);
    expect(ctx.state.wants.has(oid2)).toBe(true);
  });
  ...
  it("should return NO_WANTS when no wants sent", async () => {
    ...
    expect(ctx.state.wants.size).toBe(0);
  });
  ...
  it("should parse shallow lines", async () => {
    ...
    expect(ctx.state.clientShallow?.has(shallowOid)).toBe(true);
  });
  ...
  it("should parse deepen request", async () => {
    ...
    expect(result).toBe("WANTS_WITH_SHALLOW");
    expect(ctx.state.deepenRequest).toBe("deepen 2");
  });
  ...
  it("should parse filter request", async () => {
    ...
    expect(ctx.state.filterSpec).toBe("blob:none");
  });
});
...
describe("READ_COMMANDS FSM Handler", () => {
  it("should parse push command with capabilities", async () => {
    ...
    expect(ctx.state.capabilities.has("report-status")).toBe(true);

    const state = ctx.state as {
      pushCommands?: Array<{ oldOid: string; newOid: string; refName: string; type: string }>;
    };
  ...
  it("should detect CREATE command type", async () => {
    ...
    const state = ctx.state as { pushCommands?: Array<{ type: string }> };
  ...
  it("should parse multiple commands", async () => {
    ...
    const state = ctx.state as { pushCommands?: Array<{ refName: string }> };
    expect(state.pushCommands).toHaveLength(2);
    expect(state.pushCommands?.[0]?.refName).toBe("refs/heads/main");
    expect(state.pushCommands?.[1]?.refName).toBe("refs/heads/feature");
  });
});
