 ...
 * Pack Parser Tests
 *
 * Tests for Git pack file parsing including:
 * - Pack header parsing
 ...
 */
function parseObjectHeader(data: Uint8Array, offset: number): { type: number; size: number; headerLength: number } {
  ...
  describe("should detect corruption", () => {
    it("detects corrupted magic bytes", () => {
      const corruptedPack = new Uint8Array([
        0x00, 0x00, 0x00, 0x00, // Corrupted magic
        0x00, 0x00, 0x00, 0x02,
        0x00, 0x00, 0x00, 0x01,
      ]);
