...
import type { TransportApi, PktLineResult, SidebandResult } from "../../api/transport-api.js";
import type { RepositoryFacade, PackImportResult } from "../../api/repository-facade.js";
...
describe("Client Push FSM", () => {
  ...
  beforeEach(() => {
    ...
    refStore = createMockRefStore(
      new Map([["refs/heads/main", "abc123".padEnd(40, "0")]]),
    );
    context = {
      transport,
      repository,
  ...
  describe("READ_ADVERTISEMENT state", () => {
    it("reads refs from server advertisement", async () => {
      ...
      transport = createMockTransport([
        { type: "data", payload: new Uint8Array(0), text: `${oid1} refs/heads/main\0report-status ofs-delta\n` },
    ...
    it("handles empty repository", async () => {
      transport = createMockTransport([
        { type: "data", payload: new Uint8Array(0), text: `${ZERO_OID} capabilities^{}\0report-status\n` },
        { type: "flush" },
      ]);
      context.transport = transport;
  ...
  describe("SEND_COMMANDS state", () => {
    it("sends push commands with capabilities", async () => {
      ...
      expect(transport.written).toContain(`${oldOid} ${newOid} refs/heads/main\0report-status side-band-64k`);
...
describe("Server Push FSM", () => {
  ...
  describe("READ_COMMANDS state", () => {
    it("reads push commands from client", async () => {
      ...
      transport = createMockTransport([
        { type: "data", payload: new Uint8Array(0), text: `${oldOid} ${newOid} refs/heads/main\0report-status\n` },
        { type: "flush" },
      ]);
      context.transport = transport;
    ...
    it("detects delete command", async () => {
      ...
      transport = createMockTransport([
        { type: "data", payload: new Uint8Array(0), text: `${oldOid} ${ZERO_OID} refs/heads/feature\n` },
        { type: "flush" },
      ]);
      context.transport = transport;
