 ...
 */
async function parseInitialRequest(input: AsyncGenerator<Uint8Array>): Promise<GitProtocolRequest> {
...
/**
 * Handle a single Git socket connection over MessagePort.
 *
...
export async function handleGitSocketConnection(
  ...
  try {
    ...
    const request = await parseInitialRequest(input);
