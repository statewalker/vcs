...
import { RollingHashDeltaStrategy } from "./compute/rolling-hash-compute.js";
 ...
 */
export class DeltaStorageImpl implements DeltaStorage {
  readonly looseStorage: ObjectStorage;
  readonly deltaBackend: DeltaBackend;
  ...
  async deltifyWith(
    ...
    let bestResult: {
      baseId: ObjectId;
      delta: import("@webrun-vcs/diff").Delta[];
      ratio: number;
    } | null = null;
  ...
  async getStats(): Promise<{ loose: { count: number; size: number }; delta: DeltaBackendStats }> {
    let looseCount = 0;
    let looseSize = 0;
