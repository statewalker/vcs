...
export class RebaseCommand extends GitCommand<RebaseResult> {
   ...
   * takes that side, a path both sides changed identically merges cleanly, and
   * a path the two sides changed differently is reported as a conflict.
   *
  ...
  private async mergeTreesThreeWay(
    ...
    for (const path of allPaths) {
      ...
      if (RebaseCommand.entriesEqual(oursEntry, baseEntry)) {
        if (theirsEntry) {
          mergedEntries.set(path, theirsEntry);
        }
        continue;
      }
      if (RebaseCommand.entriesEqual(theirsEntry, baseEntry)) {
        if (oursEntry) {
      ...
      if (RebaseCommand.entriesEqual(oursEntry, theirsEntry)) {
        if (oursEntry) {
          mergedEntries.set(path, oursEntry);
        }
        continue;
