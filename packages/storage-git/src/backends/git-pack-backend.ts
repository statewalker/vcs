/**
 * Git Pack Backend implementing the DeltaBackend interface
 *
 * This provides a DeltaBackend implementation that stores objects
 * in Git pack files, using the new strategy-based delta architecture.
 *
 * Key differences from GitDeltaObjectStorage:
 * - Implements DeltaBackend interface (not DeltaObjectStorage)
 * - Works with Delta[] arrays (format-agnostic)
 * - Handles serialization to Git binary format internally
 * - Designed for composition with DeltaStorage facade
 *
 ...
 * Implements the DeltaBackend interface for use with DeltaStorage facade.
 */
export class GitPackBackend implements DeltaBackend {
