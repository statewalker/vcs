...
describe("GitDeltaCompressor", () => {
	...
	describe("computeDelta", () => {
		it("computes delta between similar content", () => {
			const base = new TextEncoder().encode(
				"Hello, World! This is the base content.",
			);
			const target = new TextEncoder().encode(
				"Hello, World! This is the target content.",
			);
	...
	describe("applyDelta", () => {
		it("applies delta to reconstruct target", () => {
			...
			const reconstructed = compressor.applyDelta(
				base,
				result?.delta as Uint8Array,
			);
		...
		it("reconstructs longer target from shorter base", () => {
			...
			const base = new TextEncoder().encode(
				"This is a sufficiently long base content for testing",
			);
			const target = new TextEncoder().encode(
		...
		it("reconstructs target from base with shared prefix", () => {
			const base = new TextEncoder().encode(
				"Longer base content with lots of extra stuff",
			);
			const target = new TextEncoder().encode(
				"Longer base content with modifications",
			);
