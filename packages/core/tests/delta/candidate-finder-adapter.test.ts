...
import type { CandidateFinder, DeltaTarget } from "../../src/delta/candidate-finder.js";
...
describe("CandidateFinderAdapter", () => {
  it("wraps CandidateFinder to work as DeltaCandidateStrategy", async () => {
    ...
    // Store target object
    await storage.store("target-id", [new TextEncoder().encode("target content")]);
  ...
  it("respects maxCandidates option", async () => {
    ...
    const adapter = new CandidateFinderAdapter(mockFinder, { maxCandidates: 5 });
    const storage = new MemoryRawStore();
...
describe("LegacyStrategyAdapter", () => {
  ...
  it("estimates similarity based on size difference", async () => {
    ...
    await storage.store("same-size", [new TextEncoder().encode("y".repeat(100))]);
    await storage.store("different-size", [new TextEncoder().encode("z".repeat(120))]);
