 ...
 */
export async function parseIndexFile(data) {
 ...
 */
function parseEntry(data, view, offset, version, previousPath) {
 ...
 */
export async function serializeIndexFile(entries, version = INDEX_VERSION_2) {
 ...
 */
function serializeEntry(entry, version, previousPath) {
 ...
 */
function serializeEntryV4(entry, pathBytes, previousPath, hasExtended) {
  // Find common prefix
  let commonLen = 0;
 ...
 */
function writeEntryHeader(view, entry, pathLength, hasExtended) {
  // ctime
 ...
 */
function readNullTerminated(data, offset) {
  let end = offset;
