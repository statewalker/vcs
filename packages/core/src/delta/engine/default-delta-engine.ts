...
export class DefaultDeltaEngine implements DeltaEngine {
	...
	async findBestDelta(target: DeltaTarget): Promise<BestDeltaResult | null> {
		...
		const targetContent =
			target.content ?? (await this.objectLoader.load(target.id));
		...
		for await (const candidate of this.candidateFinder.findCandidates(target)) {
			...
			const estimate = this.compressor.estimateDeltaQuality(
				candidate.size,
				target.size,
			);
			...
			// Compute delta
			const deltaResult = this.compressor.computeDelta(
				candidateContent,
				targetContent,
			);
	...
	async *processBatch(
		targets: AsyncIterable<DeltaTarget>,
	): AsyncIterable<DeltaProcessResult> {
...
export class SimpleObjectLoader implements ObjectLoader {
	constructor(
		...
		private readonly getChainDepthFn: (
			id: ObjectId,
