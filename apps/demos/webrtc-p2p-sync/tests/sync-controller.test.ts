...
describe.skip("SyncController (needs update for Git protocol)", () => {
  ...
  describe("full commit history sync", () => {
    it("should sync all commits when peer has more history", async () => {
      ...
      // Give extra time for all objects to be processed (more time for large sync)
    ...
    it("should have matching HEAD commit IDs after sync", async () => {
      ...
      // Find the HEAD commits by ID and compare their messages
      ...
      // After sync, HEAD commit messages should match (IDs may differ due to content-addressing)
