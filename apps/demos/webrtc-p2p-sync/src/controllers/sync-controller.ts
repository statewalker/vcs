...
import { getGitStore, getPeerConnections, setGitStore } from "./index.js";
...
export function createSyncController(ctx: AppContext): () => void {
  ...
  async function handleIncomingMessage(
    ...
    let store = getGitStore(ctx);
    const displayName = peersModel.get(peerId)?.displayName ?? peerId;

    // If no store exists and we're receiving objects, create one
    if (
      !store &&
      (message.type === "send-objects" ||
        message.type === "sync-complete" ||
        message.type === "repo-info")
    ) {
      // Request repo initialization if not already done
      ...
      // For receiving data, we need a store - create an empty one
      if (!store) {
        logModel.info("No local repository - creating empty store for sync...");
        store = await createEmptyStore(ctx);
        if (!store) {
          logModel.error("Failed to create repository for sync");
          return;
        }
      }
    }
    ...
    switch (message.type) {
      ...
      case "sync-complete": {
        ...
        logModel.success(`Received ${info?.objectCount || 0} objects from ${displayName}`);
  ...
  async function handleSyncStart(peerId: string): Promise<void> {
    ...
    // Get or create git store
    let store = getGitStore(ctx);
    if (!store) {
      // Create an empty store to receive remote data
      logModel.info("No local repository - creating empty store for sync...");
      store = await createEmptyStore(ctx);
      if (!store) {
        logModel.error("Failed to create repository for sync");
...
/**
 * Create an empty Git store for receiving sync data.
 */
async function createEmptyStore(ctx: AppContext): Promise<GitStore | null> {
  try {
