...
- `@webrun-vcs/utils` - hash, compression, diff utilities
- `@webrun-vcs/core` - VCS types, interfaces, stores
- `@webrun-vcs/transport` - HTTP operations (clone, fetch, push)
- `@webrun-vcs/commands` - high-level Git command API

**Dependencies to REMOVE:**
- `@webrun-vcs/vcs` → replace with `@webrun-vcs/core`
- `@webrun-vcs/storage-git` → replace with `@webrun-vcs/commands` (Git class) or `@webrun-vcs/core`
...
**From @webrun-vcs/vcs (replace with @webrun-vcs/core):**
- `FileMode` → `@webrun-vcs/core`
- `ObjectId` → `@webrun-vcs/core`
- `PersonIdent` → `@webrun-vcs/core`
- `ObjectType` → `@webrun-vcs/core`
- `Commit` → `@webrun-vcs/core`
- `TreeEntry` → `@webrun-vcs/core`

**From @webrun-vcs/storage-git (replace with @webrun-vcs/commands or @webrun-vcs/core):**
- `createGitRepository` → Use `Git.open()` or `Git.wrap()` from `@webrun-vcs/commands`
- `GitRepository` → Use `Git` class from `@webrun-vcs/commands`
- `createGitStorage` → Use `Git` class from `@webrun-vcs/commands`
- `PackObjectType` → Move to `@webrun-vcs/core` or access via commands
- Pack utilities → Access via `@webrun-vcs/core` if needed
...
Ensure `@webrun-vcs/core` exports all types that examples need:
...
**Verification:**
```typescript
import { FileMode, ObjectId, PersonIdent, ObjectType } from "@webrun-vcs/core";
```

### Task 1.2: Verify Commands Provides Git High-Level API

Ensure `@webrun-vcs/commands` provides the `Git` class with factory methods:
...
- **Option A:** Re-export from `@webrun-vcs/core`
- **Option B:** Re-export from `@webrun-vcs/commands`
...
The `perf-bench` app only uses `@webrun-vcs/utils`, which is in the allowed list.
...
- Update import examples to use `@webrun-vcs/core` for types
- Update import examples to use `@webrun-vcs/commands` for Git operations
...
// NEW
import { FileMode, ObjectId, PersonIdent } from "@webrun-vcs/core";
```
